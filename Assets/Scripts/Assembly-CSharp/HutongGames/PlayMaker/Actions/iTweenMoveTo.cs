			...
			if (transforms.Length >= 2)
			{
				...
				if (!reverse.IsNone && reverse.Value)
				{
					for (int i = 0; i < transforms.Length; i++)
					{
						...
						{
							tempVct3[tempVct3.Length - 1 - i] = ((space != Space.World) ? transforms[i].Value.transform.localPosition : transforms[i].Value.transform.position) + ((!vectors[i].IsNone) ? vectors[i].Value : Vector3.zero);
						}
					}
				}
				else
				{
					for (int j = 0; j < transforms.Length; j++)
					{
						...
						{
							tempVct3[j] = ((space != Space.World) ? ownerDefaultTarget.transform.parent.InverseTransformPoint(transforms[j].Value.transform.position) : transforms[j].Value.transform.position) + ((!vectors[j].IsNone) ? vectors[j].Value : Vector3.zero);
						}
					}
				}
