	...
	public class iTweenShakePosition : iTweenFsmAction
	{
		...
		private void DoiTween()
		{
			...
			if (!(ownerDefaultTarget == null))
			{
				...
				itweenType = "shake";
				iTween.ShakePosition(ownerDefaultTarget, iTween.Hash("amount", vector, "name", (!id.IsNone) ? id.Value : string.Empty, "time", (!time.IsNone) ? time.Value : 1f, "delay", (!delay.IsNone) ? delay.Value : 0f, "looptype", loopType, "oncomplete", "iTweenOnComplete", "oncompleteparams", itweenID, "onstart", "iTweenOnStart", "onstartparams", itweenID, "ignoretimescale", !realTime.IsNone && realTime.Value, "space", space, "axis", (axis != AxisRestriction.none) ? Enum.GetName(typeof(AxisRestriction), axis) : string.Empty));
			}
		}
	}
}
