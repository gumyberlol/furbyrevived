...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class SetPosition : FsmStateAction
	{
		...
		private void DoSetPosition()
		{
			...
			{
				Vector3 vector = ((!this.vector.IsNone) ? this.vector.Value : ((space != Space.World) ? ownerDefaultTarget.transform.localPosition : ownerDefaultTarget.transform.position));
				if (!x.IsNone)
				{
					vector.x = x.Value;
				}
