	...
	private void OnClick()
	{
		if (mChild == null && atlas != null && font != null && items.Count > 1)
		{
			...
			for (int count = items.Count; i < count; i++)
			{
				...
				uILabel.cachedTransform.localPosition = new Vector3(border.x + padding.x, num3, 0f);
			...
			mHighlight.cachedTransform.localScale = new Vector3(a - (border.x + padding.x) * 2f + (sprite.inner.xMin - sprite.outer.xMin) * 2f, num2 + num * 2f, 1f);
