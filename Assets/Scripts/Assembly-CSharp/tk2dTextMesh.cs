...
public class tk2dTextMesh : MonoBehaviour, ISpriteCollectionForceBuild
{
	...
	private void FormatText()
	{
		...
		StringBuilder stringBuilder = new StringBuilder(_text.Length);
		float num2 = 0f;
		float num3 = 0f;
		int num4 = -1;
		int num5 = -1;
		...
		_formattedText = stringBuilder.ToString();
	}
	...
	public void Commit()
	{
		...
		else
		{
			...
			if ((updateFlags & UpdateFlags.UpdateColors) != UpdateFlags.UpdateNone)
			{
				Color color = _color;
				Color color2 = ((!_useGradient) ? _color : _color2);
